    ...
    public function actionHistorial($empleado_id= null)
{
    ...
    $permisosUsados = PermisoEconomico::find()
...
if ($permisosDisponibles <= 0) {
    ...
    public function actionCreate($empleado_id = null)
    {
     ...
     // Ajuste para considerar solo permisos activos (status = 1) y solicitudes activas (activa = 1)
    $permisoAnterior = PermisoEconomico::find()
    ->joinWith('solicitud')
    ->where([
        'permiso_economico.empleado_id' => $empleado->id,
        'permiso_economico.status' => 1, // Solo permisos activos
...
$model->fecha_permiso_anterior = $fechaPermisoAnterior;
            ...
            $nombreFormato = 'PERMISO ECONOMICO';
...
$fecha_permiso = strftime('%A, %B %d, %Y', strtotime($model->motivoFechaPermiso->fecha_permiso));
        ...
        ->joinWith('solicitud')
        ->where([
            'permiso_economico.empleado_id' => $model->empleado->id,
            'permiso_economico.status' => 1, // Solo permisos con status 1
            'solicitud.activa' => 1 // Solo solicitudes activas
        ])
